...
class ClientRepository(BaseRepository):
    """Repository for client data persistence using SQLite via BaseRepository."""
    ...
    def exists(self, id_cliente: str) -> bool:
        ...
        with self.get_conn() as conn:
            cur = conn.execute('SELECT 1 FROM clients WHERE ID_CLIENTE = ? LIMIT 1', (str(id_cliente),))
    ...
    def get_by_id(self, id_cliente: str) -> Optional[Dict]:
        if not id_cliente:
            return None
        with self.get_conn() as conn:
            cur = conn.execute('SELECT * FROM clients WHERE ID_CLIENTE = ? LIMIT 1', (str(id_cliente),))
            row = cur.fetchone()
    ...
    def get_by_name(self, nome: str) -> List[Dict]:
        ...
        pattern = f"%{nome}%"
        with self.get_conn() as conn:
            cur = conn.execute('SELECT * FROM clients WHERE UPPER(CLIENTE) LIKE UPPER(?)', (pattern,))
    ...
    def get_by_cpf_cnpj(self, cpf_cnpj: str) -> Optional[Dict]:
        ...
        with self.get_conn() as conn:
            cur = conn.execute('SELECT * FROM clients WHERE CPF_CNPJ IS NOT NULL')
            for row in cur.fetchall():
                db_value = re.sub(r'[^0-9]', '', str(row['CPF_CNPJ'] or ''))
    ...
    def save(self, client: Client) -> bool:
        ...
        try:
            data = client.to_dict()
            # Insert using BaseRepository.insert
            self.insert(data)
            return True
        except ValueError:
            raise
    ...
    def update(self, id_cliente: str, updates: Dict) -> bool:
        ...
        try:
            ...
            # Build updated row for validations
    ...
    def get_by_vendedor(self, vendedor: str) -> List[Dict]:
        if not vendedor:
            return []
        with self.get_conn() as conn:
            cur = conn.execute('SELECT * FROM clients WHERE VENDEDOR = ? COLLATE NOCASE', (vendedor,))
            return [dict(r) for r in cur.fetchall()]
    ...
    def get_by_tipo(self, tipo: str) -> List[Dict]:
        if not tipo:
            return []
        with self.get_conn() as conn:
            cur = conn.execute('SELECT * FROM clients WHERE TIPO = ? COLLATE NOCASE', (tipo,))
            return [dict(r) for r in cur.fetchall()]

    def get_statistics(self) -> Dict:
        with self.get_conn() as conn:
            cur = conn.execute('SELECT COUNT(*) as total FROM clients')
            total = cur.fetchone()['total']
            cur = conn.execute("SELECT COUNT(*) as c FROM clients WHERE LOWER(TIPO) = 'pessoa'")
            pessoas = cur.fetchone()['c']
            cur = conn.execute("SELECT COUNT(*) as c FROM clients WHERE LOWER(TIPO) = 'empresa'")
            empresas = cur.fetchone()['c']
            cur = conn.execute('SELECT VENDEDOR, COUNT(*) as cnt FROM clients GROUP BY VENDEDOR')
